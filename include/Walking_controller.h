...
public:
  ...
  void Configure(const ControllerConfiguration & config)
  {
    ...
    Controller_Config.Beta =
        std::min(Controller_Config.Beta_range(1), std::max(Controller_Config.Beta_range(0), Controller_Config.Beta));
    Controller_Config.MPC_ZMP_Constraint_size.x() = std::min(
        Controller_Config.MPC_ZMP_Constraint_max_size,
        std::max(Controller_Config.MPC_ZMP_Constraint_min_size, Controller_Config.MPC_ZMP_Constraint_size.x()));
    Controller_Config.MPC_ZMP_Constraint_size.y() = std::min(
        Controller_Config.MPC_ZMP_Constraint_max_size,
        std::max(Controller_Config.MPC_ZMP_Constraint_min_size, Controller_Config.MPC_ZMP_Constraint_size.y()));
...
private:
  ...
  std::vector<std::vector<double>> V; // Input Reference velocity vector, contains in that order : the x-direction, the
                                      // y-directon velocity, the rotating velocity
